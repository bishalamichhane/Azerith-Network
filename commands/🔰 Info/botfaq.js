...
module.exports = {
    ...
    let es = client.settings.get(message.guild.id, "embed");let ls = client.settings.get(message.guild.id, "language")
		...
		try{
      let milratodc = client.guilds.cache.get("746006224792846366")
      let milratomembers = await milratodc.members.fetch().catch(() => {});
      let partnercount = Celestialmembers.filter(m => m.roles.cache.has("769133913746047006"))
      ...
      .setColor(es.color)
      .setAuthor(client.la[ls].cmds.info.botfaq.menuembed.title, client.user.displayAvatarURL(), "")
